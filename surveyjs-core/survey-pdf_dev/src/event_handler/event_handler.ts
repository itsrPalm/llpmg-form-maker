...
export class EventHandler {
	public static async process_header_events(
		survey: SurveyPDF,
		...
		if (!survey.haveCommercialLicense) {
			survey.onRenderHeader.add((_, canvas) => {
				canvas.drawText({
					text: "Loma Linda Psychiatric Medical Group New Patient Intake Packet",
					fontSize: 10,
				});
			});
		}
		for (let i: number = 0; i < packs.length; i++) {
			await survey.onRenderHeader.fire(
				survey,
				new DrawCanvas(
					packs[i],
					controller,
			...
			await survey.onRenderFooter.fire(
				survey,
				new DrawCanvas(
					packs[i],
					controller,
